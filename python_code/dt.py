    ...
    while True:
        try:
            ...
            for line in lines:
                ...
                if len(parts) < 1:  # 데이터 포인트가 1개이므로 길이 체크를 1로 수정
                    continue

                # 데이터 포인트를 공백으로 구분하여 가져오기
                data_points = parts  # 이전 형식과 다르게 이제 parts 전체가 데이터 포인트

                if len(data_points) > POINT_INDEX:  # 이제 하나의 데이터 포인트 필요
                    # 데이터 포인트 선택
                    selected_point = data_points[POINT_INDEX]

                    # 데이터 포인트를 쉼표로 분리
                    values = selected_point.split(',')
                    
                    if len(values) == 3:
                        try:
                            data_0 = float(values[0])
