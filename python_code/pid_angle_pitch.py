...
POINT_0_INDEX = 0
POINT_1_INDEX = 0
POINT_2_INDEX = 0  # 세 번째 데이터 포인트 인덱스 추가

# 인덱스 정의 (쉼표로 구분된 데이터 포인트 내 인덱스)
index_0 = 0
index_1 = 1
index_2 = 2  # 세 번째 데이터 포인트 인덱스 추가

# 데이터 이름을 변수로 지정
DATA_NAME_0 = 'Target'
DATA_NAME_1 = 'Input'
DATA_NAME_2 = 'Additional'  # 세 번째 데이터 포인트의 데이터 이름
    ...
    while True:
        try:
            ...
            for line in lines:
                ...
                if len(parts) < 3:  # 데이터 포인트가 3개 이상이므로 길이 체크를 3으로 수정
                    continue

                # 데이터 포인트를 공백으로 구분하여 가져오기
                data_points = parts  # 이제 parts 전체가 데이터 포인트

                if len(data_points) > POINT_2_INDEX:  # 세 개의 데이터 포인트 필요
                    # 0번째, 1번째, 2번째 데이터 포인트 선택
                    selected_point_0 = data_points[POINT_0_INDEX]
                    selected_point_1 = data_points[POINT_1_INDEX]
                    selected_point_2 = data_points[POINT_2_INDEX]

                    # 데이터 포인트를 쉼표로 분리
                    values_0 = selected_point_0.split(',')
                    values_1 = selected_point_1.split(',')
                    values_2 = selected_point_2.split(',')
                    
                    if len(values_0) == 3 and len(values_1) == 3 and len(values_2) == 3:
                        try:
                            data_0 = float(values_0[index_0])
                            data_1 = float(values_1[index_1])
                            data_2 = float(values_2[index_2])
                            
                            # 추출된 데이터 출력
                            print(f"추출된 데이터: Cycle={cycle_counter}, {DATA_NAME_0}={data_0}, {DATA_NAME_1}={data_1}, {DATA_NAME_2}={data_2}")
...
# 실시간 업데이트 함수
def update_plot(frame, line_0, line_1, line_2, ax):
    ...
    line_2.set_data(df['Cycle'], df[DATA_NAME_2])
    ...
    # y축 범위 설정
    ax.set_ylim(-200, 200)  # y축 범위를 -600에서 600으로 설정
...
line_2, = ax.plot([], [], lw=2, label=DATA_NAME_2, color='green')  # 세 번째 데이터 포인트 추가

# 애니메이션 설정
ani = animation.FuncAnimation(
    fig, 
    update_plot, 
    fargs=(line_0, line_1, line_2, ax), 
